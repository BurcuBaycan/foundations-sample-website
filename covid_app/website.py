...
def create_meeting():
    # try:
    # define today and 2 weeks ago
    date = datetime.today().strftime('%Y-%m-%d')
    #twoWeeks = timedelta(weeks=2)
    #two_weeks_ago = today - twoWeeks
    ...
    sql_insert = "INSERT INTO Meetings (name, date, location, contact, duration) VALUES (\"{name}\", \"{date}\", \"{location}\",\"{duration}\",\"{contact}\");".format(
        name=name, date=date, location=location, duration=duration, contact=contact)
    ...
    sql_query = "SELECT * FROM Meetings WHERE date BETWEEN datetime('NOW', '-14 days') AND datetime ('NOW') ORDER BY date DESC, name ASC;"
